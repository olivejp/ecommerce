...
public class AttributArticleResource {
    ...
    public AttributArticleResource(AttributArticleRepository attributArticleRepository, AttributArticleSearchRepository attributArticleSearchRepository) {
     ...
     *
     * @param attributArticle the attributArticle to create.
     * @return the {@link ResponseEntity} with status {@code 201 (Created)} and with body the new attributArticle, or with status {@code 400 (Bad Request)} if the attributArticle has already an ID.
     * @throws URISyntaxException if the Location URI syntax is incorrect.
     */
    @PostMapping("/attribut-articles")
    public ResponseEntity<AttributArticle> createAttributArticle(@RequestBody AttributArticle attributArticle) throws URISyntaxException {
        log.debug("REST request to save AttributArticle : {}", attributArticle);
        if (attributArticle.getId() != null) {
            throw new BadRequestAlertException("A new attributArticle cannot already have an ID", ENTITY_NAME, "idexists");
        }
        AttributArticle result = attributArticleRepository.save(attributArticle);
        attributArticleSearchRepository.save(result);
     ...
     *
     * @param attributArticle the attributArticle to update.
     * @return the {@link ResponseEntity} with status {@code 200 (OK)} and with body the updated attributArticle,
     * or with status {@code 400 (Bad Request)} if the attributArticle is not valid,
     * or with status {@code 500 (Internal Server Error)} if the attributArticle couldn't be updated.
     * @throws URISyntaxException if the Location URI syntax is incorrect.
     */
    @PutMapping("/attribut-articles")
    public ResponseEntity<AttributArticle> updateAttributArticle(@RequestBody AttributArticle attributArticle) throws URISyntaxException {
        log.debug("REST request to update AttributArticle : {}", attributArticle);
        if (attributArticle.getId() == null) {
        ...
        return ResponseEntity.ok()
            .headers(HeaderUtil.createEntityUpdateAlert(applicationName, true, ENTITY_NAME, attributArticle.getId().toString()))
            .body(result);
    }
    ...
    public List<AttributArticle> getAllAttributArticles() {
        log.debug("REST request to get all AttributArticles");
        return attributArticleRepository.findAll();
    }

    /**
     * {@code GET  /attribut-articles/:id} : get the "id" attributArticle.
     *
     * @param id the id of the attributArticle to retrieve.
     * @return the {@link ResponseEntity} with status {@code 200 (OK)} and with body the attributArticle, or with status {@code 404 (Not Found)}.
     */
    @GetMapping("/attribut-articles/{id}")
    public ResponseEntity<AttributArticle> getAttributArticle(@PathVariable Long id) {
        log.debug("REST request to get AttributArticle : {}", id);
        Optional<AttributArticle> attributArticle = attributArticleRepository.findById(id);
        return ResponseUtil.wrapOrNotFound(attributArticle);
    }

    /**
     * {@code DELETE  /attribut-articles/:id} : delete the "id" attributArticle.
     *
     * @param id the id of the attributArticle to delete.
    ...
    public List<AttributArticle> searchAttributArticles(@RequestParam String query) {
