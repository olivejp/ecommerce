...
export class CommandeService {
  ...
  protected convertDateFromClient(commande: ICommande): ICommande {
    const copy: ICommande = Object.assign({}, commande, {
      date: commande.date && commande.date.isValid() ? commande.date.toJSON() : undefined,
      datePayment: commande.datePayment && commande.datePayment.isValid() ? commande.datePayment.toJSON() : undefined,
    });
    return copy;
  }
  ...
  protected convertDateFromServer(res: EntityResponseType): EntityResponseType {
    if (res.body) {
      ...
      res.body.datePayment = res.body.datePayment ? moment(res.body.datePayment) : undefined;
    }
    return res;
  }
  ...
  protected convertDateArrayFromServer(res: EntityArrayResponseType): EntityArrayResponseType {
    if (res.body) {
      res.body.forEach((commande: ICommande) => {
        ...
        commande.datePayment = commande.datePayment ? moment(commande.datePayment) : undefined;
      });
    }
    return res;
  }
